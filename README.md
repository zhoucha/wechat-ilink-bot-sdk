...
if(!sdk.
    ...
    System.out.
        ...
        println(qrcode.getQrcodeImgContent());

        // 等待用户扫码
        sdk.

        loginWithQrcode(qrcode.getQrcode(), new QrcodeLoginManager.
        ...
        QrcodeStatusCallback() {
            ...
            public void onScanned () {
                System.out.println("Code scanned!");
            }
...
// 添加消息监听器
        ...
        addMessageListener(new SimpleMessageListener() {
            @Override
...
setLongPollingTimeoutMs(35000);   // 长轮询超时: 35秒
config.

setSendTimeoutMs(15000);         // 发送超时: 15秒
config.

setMaxConsecutiveFailures(3);    // 最大连续失败次数
config.

setRetryDelayMs(2000);           // 重试延迟: 2秒
config.

setBackoffDelayMs(30000);        // 退避延迟: 30秒
